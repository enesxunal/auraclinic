...
function buildUserData(body, req) {
  ...
  if (body.city) {
    var cityNorm = normText(body.city);
    if (cityNorm) userData.ct = sha256(cityNorm);
  }
  if (body.country) {
    var countryNorm = String(body.country || "").trim().toLowerCase().replace(/[^a-z]/g, "");
    if (countryNorm) userData.country = sha256(countryNorm);
  }
...
async function sendLeadEvents(body, req) {
  ...
  // Browser + CAPI: Lead only (no CompleteRegistration for form flow)
  var events = [
    cleanObject({
      event_name: "Lead",
      event_time: now,
